...
describe('mysql 适配器（离线负向用例）', () => {
  ...
  it('ro 模式：白名单首词（含大小写/空白）不被适配器层拒绝（此后才触达服务器）', async () => {
    ...
    try {
      ...
      // 双断言都必须执行：若白名单误拦（回归），message 会含「只读」→ 第 2 条失败
      const err: unknown = await a.query('  select 1').then(
        () => { throw new Error('离线环境 query 应 reject（连接失败）'); },
        (e: unknown) => e,
      );
      const msg = err instanceof Error ? err.message : String(err);
      expect(msg).toMatch(/mysql 查询失败/);
  ...
  it('defaultDb：URL pathname 提供默认库；特殊字符库名被安全引用（不再误杀）', async () => {
    ...
    try {
      // 注意 label 是「mysql 列出表」（listTables 的 humanize 标签）；双断言防回归：
      // 若恢复旧字符白名单（误杀 db`x），message 会含「非法」→ 第 2 条失败
      const err: unknown = await a.listTables().then(
        () => { throw new Error('离线环境 listTables 应 reject（连接失败）'); },
        (e: unknown) => e,
      );
      const msg = err instanceof Error ? err.message : String(err);
      expect(msg).toMatch(/mysql 列出表/);
