...
| `GET /api/preview?project=<path>&connId=&database=&table=&limit=` | QueryResult（limit≤50 强制） |
...
| `POST /api/script` | `{ projectPath, connId, code, challengeId? }` → vm 沙箱执行 run_script 同语义 |
